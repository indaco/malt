...
const install_help =
    ...
    \\                     install with TLS as its only integrity check. Covers every
    \\                     package the run installs, dependencies included, and warns
    \\                     on each; an unpinned .pkg is still refused.
...
const upgrade_help =
    ...
    \\override. Kegs from `mt install --local` have no upstream and are
    \\skipped too; re-run the install to update one. malt follows the
    \\tap: if the tap version differs from the installed one it is
    \\applied, even if that moves the version down (e.g. an upstream
    \\revert). A backward move is announced with a warning
...
const restore_help =
    ...
    \\  --quiet, -q   Suppress non-error output
    \\
    \\A tap or local recipe that declares `sha256 :no_check` is never installed:
    \\a backup file cannot opt in to `--allow-unpinned`.
    \\
    \\Example:
