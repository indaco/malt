//! malt — Installed tab for `mt tui` (placeholder; data lands in a later layer).
...
pub fn step(s: *State, key: tab.Key) void {
    _ = s; // no domain keys yet — the shell owns navigation/filter
    _ = key;
}
...
const placeholder = [_][]const u8{"Installed — no data yet"};
