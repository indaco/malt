...
test "HttpClient.getResolved: returns OOM with no leak when initial url dupe fails" {
    var failing = std.testing.FailingAllocator.init(testing.allocator, .{ .fail_index = 0 });
    var http = client.HttpClient.init(std.Options.debug_io, std.process.Environ.empty, failing.allocator());
    defer http.deinit();

    try testing.expectError(error.OutOfMemory, http.getResolved("https://example.com/"));
}
