...
# `write` truncated the user's file before swallowing the failed write.
#
...
# Always rebuild so the binary reflects current source. The env scrub keeps a
# caller's MALT_* overrides out of the tests.
