...
Only directly-installed packages are recorded; transitive dependencies are resolved on restore. The file format is one entry per line (`formula <name>` / `cask <token>`, with tap packages as `<user>/<repo>/<name>`) with `#` comments. Restore batches into two `mt install` invocations, so dependency resolution, parallel downloads, and atomic install all apply. A recorded version is informational: restore installs the current release. A `--local` keg is kept as a `# local <name> <path>` comment, and restore prints the command to rebuild it. A local keg whose name or recipe path holds a line break is left out of the backup, with a warning.
