...
| `MALT_THEME`                    | Colour theme for all output (CLI and `mt tui`): `light`/`dark`/`auto` pick the background-aware default (OSC 11 detect); `dracula`, `catppuccin-mocha`/`-latte`, `rose-pine`/`-dawn`, `nord`, `tokyo-night`, `gruvbox-dark`/`-light` apply on a truecolor terminal whose background matches, else fall back to the default palette | `auto`                         |
