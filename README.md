...
`mt reinstall <pkg>` is the discoverable peer of `mt install --force`: refuses if the package isn't installed, wipes and re-materialises the existing keg or cask. A tap package reinstalls from its owning tap, named bare or as `<user>/<repo>/<name>`, on its own. Formulae and casks are reinstalled in separate runs. Transitive dependencies are not reinstalled. Global flags (`--json`, `--quiet`, `--dry-run`) pass through.
