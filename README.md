...
| `MALT_THEME`                    | Output background palette (`light`/`dark`/`auto`, OSC 11 detect) and the `mt tui` theme: `dracula`, `catppuccin-mocha`/`-latte`, `rose-pine`/`-dawn`, `nord`, `tokyo-night`, `gruvbox-dark`/`-light` (truecolor; else basic) | `auto`                         |
